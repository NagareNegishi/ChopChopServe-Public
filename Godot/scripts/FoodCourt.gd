...
const MIN_CUSTOMER_DELAY: float = 10
const QUEUE_CHECK_DELAY: float = 0.5
const SPAWN_ACCELERATION: float = 0.05
...
func _process(delta: float):
	...
	if game_state.current_phase == GameStateTest.Phases.SERVE:
		current_max_delay = max(12, current_max_delay - delta * SPAWN_ACCELERATION)
		current_min_delay = max(6, current_min_delay -  delta * SPAWN_ACCELERATION)
